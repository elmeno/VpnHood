...
export default {
  ...
  async loadInstalledApps () {
    ...
    this.installedApps = ret.sort((a, b) =>
  ...
  async loadApp (options) {
    ...
    if (options.withSettings) {
      ...
      this.userSettings = data.settings.userSettings
    }
    if (options.withClientProfileItems)
      this.clientProfileItems = data.clientProfileItems
  ...
  async invokeApi (method, args = {}) {
    ...
    if (response.status != 200)
      throw `Failed to call the ${method} api. status: ${response.status}`
    return response
  },
